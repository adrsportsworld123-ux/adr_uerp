...
3. On-device verification of the offline sync flow (see above), of the printer integration, and of the 2026-09-22 Flutter parity pass — real Android/iOS hardware and a real network thermal printer have never been available in this environment; everything above is verified against a real backend over HTTP/TCP instead, which is meaningfully better than a stub but not the same as an actual cashier's hands on an actual device
...
**UI, built in this same pass:** `erp-web-admin`'s Notifications screen (`/notifications`) — the sent-notification audit log and low-stock reorder-point configuration (this app has no general Inventory screen yet, so reorder-point setting lives here rather than there). The first screen in this app whose *read*, not just its writes, is permission-gated — a denied user sees the API's own permission message rather than a blank page. Verified with a real Playwright test (`e2e/notifications.spec.ts`) that seeds a real finalized sale via a direct API call to populate the log, sets a reorder point end-to-end, and confirms a POS User is denied the log (but can still look up stock) and denied saving a reorder point.
...
**Phase 4's Sub-area 4, Reconciliation & Audit, is now fully complete across all three reconciliation types this environment can build without a vendor decision** — Cash, Payment Gateway, and Inventory, backend and UI both. Audit trail hardening (immutable/checksummed, 7-year retention) is tracked separately, not a reconciliation type, and remains open.

**Not yet started:** E-Invoicing/E-Way Bill (needs a GSP vendor decision — see this phase's own kickoff discussion), and audit trail hardening (immutable/checksummed, 7-year retention — `audit_logs` has existed as a partitioned table since Phase 0/1, but its immutability/checksumming and formal 7-year retention policy were never built out). Formal PO approval workflow remains genuinely optional per this phase's own "if you have suppliers who need it" hedge.
...
| 1 | Core retail loop (real MVP) | XL | Functionally complete — 3 small open items above (standalone reservations endpoint, forgot-password flow, on-device hardware verification) |
...
| 4 | GST compliance depth, Audit | L | In progress — Credit Facility, GST Returns, Bank/Payment Gateway/Inventory Reconciliation all done; E-Invoicing/E-Way Bill (vendor decision) and audit trail hardening remain, see above |
